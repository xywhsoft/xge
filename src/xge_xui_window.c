...
static void __xgeXuiWindowSyncButtonText(xge_xui_window pWindow)
{
	...
	xgeXuiButtonSetText(&pWindow->tCollapseButton, pWindow->pFont, pWindow->bCollapsed ? "+" : "-");
	xgeXuiButtonSetText(&pWindow->tMaximizeButton, pWindow->pFont, pWindow->bMaximized ? "[]" : "[ ]");
	xgeXuiButtonSetText(&pWindow->tCloseButton, pWindow->pFont, "X");
}
...
static void __xgeXuiWindowSyncButtonColors(xge_xui_window pWindow)
{
	...
	xgeXuiButtonSetColors(&pWindow->tCloseButton,
		XGE_COLOR_RGBA(96, 38, 52, 214),
		XGE_COLOR_RGBA(176, 64, 86, 236),
		XGE_COLOR_RGBA(72, 24, 36, 240),
		XGE_COLOR_RGBA(176, 64, 86, 236),
		XGE_COLOR_RGBA(96, 38, 52, 128));
...
int xgeXuiWindowInit(xge_xui_window pWindow, xge_xui_context pContext, xge_xui_widget pWidget)
{
	...
	pWindow->sTitle = "";
	pWindow->iBackgroundColor = XGE_COLOR_RGBA(28, 34, 44, 255);
	pWindow->iTitleBarColor = XGE_COLOR_RGBA(44, 90, 156, 255);
	pWindow->iTitleTextColor = XGE_COLOR_RGBA(244, 247, 251, 255);
	pWindow->iBorderColor = XGE_COLOR_RGBA(78, 96, 124, 255);
	pWindow->iButtonColorNormal = XGE_COLOR_RGBA(255, 255, 255, 24);
	pWindow->iButtonColorHover = XGE_COLOR_RGBA(255, 255, 255, 54);
	pWindow->iButtonColorActive = XGE_COLOR_RGBA(0, 0, 0, 70);
	pWindow->fTitleBarHeight = 28.0f;
	pWindow->fBorderWidth = 1.0f;
