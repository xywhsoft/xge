...
void xgeXuiFontDestroy(xge_xui_context pContext, xui_font pFont)
{
	...
	if ( iIndex != iLast ) {
		...
		memmove(pContext->arrFontName[iIndex], pContext->arrFontName[iLast], sizeof(pContext->arrFontName[iIndex]));
		pContext->arrFontName[iIndex][sizeof(pContext->arrFontName[iIndex]) - 1] = 0;
	}
	pContext->iFontCount--;
}
...
static void __xgeXuiWidgetTooltipDefaultDesc(xge_xui_tooltip_desc_t* pDesc)
{
	if ( pDesc == NULL ) {
		return;
	}
...
static int __xgeXuiWidgetTooltipDescEnabled(const xge_xui_tooltip_desc_t* pDesc)
{
	if ( pDesc == NULL ) {
		return 0;
	}
...
static void __xgeXuiWidgetTooltipNormalizeDesc(xge_xui_tooltip_desc_t* pDesc)
{
	if ( pDesc == NULL ) {
		return;
	}
...
static int __xgeXuiWidgetTooltipTextSame(const char* sLeft, const char* sRight)
{
	if ( sLeft == sRight ) {
		return 1;
	}
...
static int __xgeXuiWidgetTooltipDescSame(const xge_xui_tooltip_desc_t* pLeft, const xge_xui_tooltip_desc_t* pRight)
{
	if ( pLeft == pRight ) {
		return 1;
	}
...
static int __xgeXuiWidgetTooltipResolveWidget(xge_xui_context pContext, xge_xui_widget pWidget, xge_xui_tooltip_desc_t* pDesc)
{
	if ( pDesc == NULL ) {
		return 0;
	}
	__xgeXuiWidgetTooltipDefaultDesc(pDesc);
	if ( pWidget == NULL ) {
		return 0;
	}
	if ( pWidget->procTooltipResolve != NULL ) {
		...
		__xgeXuiWidgetTooltipNormalizeDesc(pDesc);
		if ( __xgeXuiWidgetTooltipDescEnabled(pDesc) ) {
			return 1;
		}
		__xgeXuiWidgetTooltipDefaultDesc(pDesc);
		return 0;
...
static xge_xui_widget __xgeXuiWidgetTooltipFindOwner(xge_xui_context pContext, xge_xui_widget pWidget, xge_xui_tooltip_desc_t* pDesc)
{
	xge_xui_tooltip_desc_t tDesc;

	for ( ; pWidget != NULL; pWidget = pWidget->pParent ) {
		if ( __xgeXuiWidgetTooltipResolveWidget(pContext, pWidget, &tDesc) ) {
			if ( pDesc != NULL ) {
				*pDesc = tDesc;
			}
			return pWidget;
...
static void __xgeXuiWidgetTooltipClose(xge_xui_context pContext)
{
	if ( pContext == NULL ) {
		return;
	}
...
static float __xgeXuiWidgetTooltipClamp(float fValue, float fMin, float fMax)
{
	if ( fMax < fMin ) {
		return fMin;
	}
...
static xge_rect_t __xgeXuiWidgetTooltipResolveRect(xge_xui_context pContext, xge_xui_widget pOwner, const xge_xui_tooltip_desc_t* pDesc, xge_vec2_t tSize)
{
	xge_rect_t tRect;
	xge_rect_t tRoot;
	xge_rect_t tOwner;
	...
	tRect.fX = __xgeXuiWidgetTooltipClamp(tRect.fX, tRoot.fX + fMargin, tRoot.fX + tRoot.fW - tRect.fW - fMargin);
	tRect.fY = __xgeXuiWidgetTooltipClamp(tRect.fY, tRoot.fY + fMargin, tRoot.fY + tRoot.fH - tRect.fH - fMargin);
	return tRect;
}

static void __xgeXuiWidgetTooltipOpen(xge_xui_context pContext)
{
	xge_vec2_t tSize;
	xge_rect_t tRect;
	...
	if ( !__xgeXuiWidgetTooltipDescEnabled(pDesc) || !xgeXuiWidgetIsVisible(pContext->pTooltipOwner) || !xgeXuiWidgetIsEnabled(pContext->pTooltipOwner) ) {
		__xgeXuiWidgetTooltipClose(pContext);
		return;
	}
	tSize = __xgeXuiWidgetTooltipMeasure(pContext, pContext->pTooltipOwner, pDesc);
	tRect = __xgeXuiWidgetTooltipResolveRect(pContext, pContext->pTooltipOwner, pDesc, tSize);
...
void xgeXuiWidgetSetTooltipText(xge_xui_widget pWidget, const char* sText)
{
	...
	__xgeXuiWidgetTooltipDefaultDesc(&tDesc);
...
void xgeXuiWidgetSetTooltip(xge_xui_widget pWidget, const xge_xui_tooltip_desc_t* pDesc)
{
	...
	__xgeXuiWidgetTooltipNormalizeDesc(&tDesc);
	if ( !__xgeXuiWidgetTooltipDescEnabled(&tDesc) ) {
		xgeXuiWidgetClearTooltip(pWidget);
		return;
	}
...
void xgeXuiWidgetSetTooltipResolver(xge_xui_widget pWidget, xge_xui_tooltip_resolve_proc procResolve, void* pUser)
{
	...
	__xgeXuiWidgetTooltipDefaultDesc(&pWidget->tTooltip);
...
static void __xgeXuiWidgetTooltipHandleEvent(xge_xui_context pContext, xge_xui_widget pHit, const xge_event_t* pEvent)
{
	...
	switch ( pEvent->iType ) {
		...
		case XGE_EVENT_TOUCH_MOVE:
			...
			if ( xgeXuiHasCapture(pContext) ) {
				...
				pOwner = __xgeXuiWidgetTooltipFindOwner(pContext, pHit, &tDesc);
			}
			if ( pOwner != pContext->pTooltipOwner ) {
				...
				if ( pOwner != NULL ) {
					...
					__xgeXuiWidgetTooltipDefaultDesc(&pContext->tActiveTooltip);
				}
			} else if ( pOwner != NULL && !__xgeXuiWidgetTooltipDescSame(&pContext->tActiveTooltip, &tDesc) ) {
...
static void __xgeXuiWidgetTooltipUpdate(xge_xui_context pContext, float fDelta)
{
	xge_xui_tooltip_desc_t tDesc;
	...
	if ( !__xgeXuiWidgetTooltipResolveWidget(pContext, pContext->pTooltipOwner, &tDesc) || !xgeXuiWidgetIsVisible(pContext->pTooltipOwner) || !xgeXuiWidgetIsEnabled(pContext->pTooltipOwner) ) {
		pContext->pTooltipOwner = NULL;
		pContext->fTooltipHoverTime = 0.0f;
		__xgeXuiWidgetTooltipDefaultDesc(&pContext->tActiveTooltip);
		__xgeXuiWidgetTooltipClose(pContext);
	...
	if ( !__xgeXuiWidgetTooltipDescSame(&pContext->tActiveTooltip, &tDesc) ) {
		pContext->tActiveTooltip = tDesc;
		if ( pContext->bTooltipOpen != 0 ) {
			__xgeXuiWidgetTooltipOpen(pContext);
		} else {
...
static void __xgeXuiWidgetTooltipPaintProc(xge_xui_widget pWidget, void* pUser)
{
	xge_xui_context pContext;
	xge_rect_t tRect;
	xge_rect_t tText;
...
int xgeXuiInit(xge_xui_context pContext)
{
	...
	pContext->pTooltipPopupWidget->procPaint = __xgeXuiWidgetTooltipPaintProc;
...
int xgeXuiDispatchEvent(xge_xui_context pContext, const xge_event_t* pEvent)
{
	...
	if ( bHitRequired ) {
		__xgeXuiWidgetTooltipHandleEvent(pContext, pHit, pEvent);
	}
...
int xgeXuiUpdate(xge_xui_context pContext, float fDelta)
{
	...
	if ( pContext->pOverlayRoot != NULL ) {
		...
		__xgeXuiWidgetTooltipUpdate(pContext, fDelta);
	}
