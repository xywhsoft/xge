param(
	...
	[string]$OutputDir = "artifacts\svg_radial_gpu_perf",
	[switch]$SkipBuild
)
...
$asset = Join-Path $root "examples\xge_svg\assets\radial_gradient_gpu_batch_41.svg"
...
$capture = Join-Path $outputFull "radial_gradient_gpu_batch_41.png"
$samples = @()
Push-Location $root
try {
	for ($run = 1; $run -le $Runs; $run++) {
		...
		try {
			$output = & $exe --render $asset --capture $capture --width 500 --height 220 --aspect none --repeat $Repeat 2>&1
		} finally {
...
$manifest = [ordered]@{
	...
	asset = "examples\xge_svg\assets\radial_gradient_gpu_batch_41.svg"
	width = 500
	height = 220
...
Write-Host ("SVG radial GPU perf: median={0:F3} ms mean={1:F3} ms min={2:F3} ms max={3:F3} ms" -f $median, $mean, $sorted[0], $sorted[$sorted.Count - 1])
Write-Host "SVG radial GPU perf manifest: $manifestPath"
if (-not $passed) {
	throw ("SVG radial GPU median {0:F3} ms exceeds limit {1:F3} ms." -f $median, $MaxMedianMs)
}
