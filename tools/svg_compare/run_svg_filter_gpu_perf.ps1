param(
	...
	[string]$OutputDir = "artifacts\svg_filter_gpu_perf",
	[switch]$SkipBuild
)
...
$assetRelative = "examples\xge_svg\assets\filter_gaussian_sigma_rounding.svg"
...
try {
	for ($run = 1; $run -le $Runs; $run++) {
		$capture = Join-Path $outputFull ("filter_gaussian_sigma_rounding_run_{0}.png" -f $run)
...
$uniqueHashes = @($captureHashes | Sort-Object -Unique)
$hashesStable = $uniqueHashes.Count -eq 1
$withinLimit = ($MaxMedianMs -lt 0.0) -or ($median -le $MaxMedianMs)
$passed = $hashesStable -and $withinLimit
$manifest = [ordered]@{
	...
	summary = [ordered]@{
		...
		capture_hash = if ($hashesStable) { $uniqueHashes[0] } else { $null }
		capture_hashes_stable = $hashesStable
		passed = $passed
	}
}
$manifestPath = Join-Path $outputFull "manifest.json"
$manifest | ConvertTo-Json -Depth 8 | Set-Content -LiteralPath $manifestPath -Encoding UTF8
Write-Host ("SVG filter GPU perf: median={0:F3} ms mean={1:F3} ms min={2:F3} ms max={3:F3} ms stable_hash={4}" -f $median, $mean, $sorted[0], $sorted[$sorted.Count - 1], $hashesStable)
Write-Host "SVG filter GPU perf manifest: $manifestPath"
if (-not $hashesStable) {
	throw "SVG filter captures changed across independent process lifecycles."
}
if (-not $withinLimit) {
	throw ("SVG filter median {0:F3} ms exceeds limit {1:F3} ms." -f $median, $MaxMedianMs)
}
