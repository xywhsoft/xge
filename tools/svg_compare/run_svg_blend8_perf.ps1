param(
	...
	[string]$OutputDir = "artifacts\svg_blend8_perf",
	[switch]$SkipBuild
)
...
$assetRelative = "examples\xge_svg\assets\blend_fill_stroke_gradients.svg"
...
$capture = Join-Path $outputFull "blend_fill_stroke_gradients.png"
$samples = @()
Push-Location $root
try {
...
Write-Host ("SVG blend8 perf: median={0:F3} ms mean={1:F3} ms min={2:F3} ms max={3:F3} ms" -f $median, $mean, $sorted[0], $sorted[$sorted.Count - 1])
Write-Host "SVG blend8 perf manifest: $manifestPath"
if (-not $passed) {
	throw ("SVG blend8 median {0:F3} ms exceeds limit {1:F3} ms." -f $median, $MaxMedianMs)
}
