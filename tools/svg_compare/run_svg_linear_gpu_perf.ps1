param(
	...
	[string]$OutputDir = "artifacts\svg_linear_gpu_perf",
	[switch]$SkipBuild
)
...
$assetRelative = "examples\xge_svg\assets\gradient_repeat_seam_batch_39.svg"
...
$capture = Join-Path $outputFull "gradient_repeat_seam_batch_39.png"
$samples = @()
Push-Location $root
try {
...
Write-Host ("SVG linear GPU perf: median={0:F3} ms mean={1:F3} ms min={2:F3} ms max={3:F3} ms" -f $median, $mean, $sorted[0], $sorted[$sorted.Count - 1])
Write-Host "SVG linear GPU perf manifest: $manifestPath"
if (-not $passed) {
	throw ("SVG linear GPU median {0:F3} ms exceeds limit {1:F3} ms." -f $median, $MaxMedianMs)
}
