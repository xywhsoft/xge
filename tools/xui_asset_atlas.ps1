param(
	...
	[string]$Out = "src/xge_xui_builtin_assets.inc"
)
...
function Write-Codegen($Config, [string]$PngPath, [string]$OutputPath) {
	...
	[void]$sb.AppendLine("#ifndef XGE_XUI_BUILTIN_ASSETS_INC")
	[void]$sb.AppendLine("#define XGE_XUI_BUILTIN_ASSETS_INC")
	[void]$sb.AppendLine("")
	[void]$sb.AppendLine("#define XGE_XUI_BUILTIN_ATLAS_WIDTH $([int]$Config.width)")
	[void]$sb.AppendLine("#define XGE_XUI_BUILTIN_ATLAS_HEIGHT $([int]$Config.height)")
	[void]$sb.AppendLine("#define XGE_XUI_BUILTIN_ASSET_COUNT $($Config.assets.Count)")
	[void]$sb.AppendLine("")

	$index = 0
	foreach ($asset in $Config.assets) {
		$macro = "XGE_XUI_ASSET_$(Convert-AssetName ([string]$asset.name))"
	...
	[void]$sb.AppendLine("")
	[void]$sb.AppendLine("static const xge_xui_builtin_asset_t g_arrXgeXuiBuiltinAssets[XGE_XUI_BUILTIN_ASSET_COUNT] = {")
	foreach ($asset in $Config.assets) {
		[void]$sb.AppendLine("	{ ""$($asset.name)"", $([int]$asset.x), $([int]$asset.y), $([int]$asset.w), $([int]$asset.h) },")
	}
	[void]$sb.AppendLine("};")
	[void]$sb.AppendLine("")
	[void]$sb.AppendLine("static const unsigned char g_arrXgeXuiBuiltinAtlasPng[] = {")
	...
	[void]$sb.AppendLine("};")
	[void]$sb.AppendLine("static const int g_iXgeXuiBuiltinAtlasPngSize = (int)sizeof(g_arrXgeXuiBuiltinAtlasPng);")
	[void]$sb.AppendLine("")
