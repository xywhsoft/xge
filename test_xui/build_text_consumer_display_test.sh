...
cd "$(dirname "$0")/.."
CC="${CC:-gcc}"
OUT="${XUI_TEXT_CONSUMER_DISPLAY_TEST_OUT:-build/xui_text_consumer_display_asan}"
mkdir -p build
"$CC" -O1 -g -fsanitize=address,undefined -fno-omit-frame-pointer -no-pie \
    ...
    -DXGE_DEBUGMODE=0 -ffunction-sections -fdata-sections -I. -o "$OUT" \
    test_xui/xui_text_consumer_display_test.c test_xui/xui_test_proxy.c test_xui/xui_test_xrt_impl.c \
    ...
    src/xui_scroll_frame.c src/xui_scroll_view.c -Wl,--gc-sections -lm -ldl -lpthread
