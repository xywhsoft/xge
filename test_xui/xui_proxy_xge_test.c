...
static const char* __xuiTestCjkFontPath(void)
{
	static const char* arrPaths[] = {
...
static float __xuiTestTextVisualRight(xge_font pFont, const char* sText)
{
	...
	while ( *sScan != '\0' ) {
		...
		if ( (iCodepoint == '\r') || (iCodepoint == '\n') ) {
			break;
		}
		if ( xgeFontGlyphGet(pFont, iCodepoint, &tMetrics) == XGE_OK ) {
			fGlyphRight = fPenX + tMetrics.fX1;
			if ( fGlyphRight > fRight ) fRight = fGlyphRight;
...
static int __xuiTestSurface(xui_proxy pProxy)
{
	...
	memset(&tRawFont, 0, sizeof(tRawFont));
	...
	if ( sFontPath != NULL ) {
		...
		sCjkFontPath = __xuiTestCjkFontPath();
		if ( sCjkFontPath != NULL ) {
			memset(&tRawFont, 0, sizeof(tRawFont));
			iRet = xgeFontLoad(&tRawFont, sCjkFontPath, 18.0f);
			if ( iRet == XGE_OK ) {
				tRawTextSize = xgeTextMeasure(&tRawFont, "ADB 连接失败");
				fVisualRight = __xuiTestTextVisualRight(&tRawFont, "ADB 连接失败");
				if ( (tRawTextSize.fX + 0.01f) < fVisualRight ) {
					printf("xui_proxy_xge_test failed: text measure clips glyphs size=%f visualRight=%f\n", tRawTextSize.fX, fVisualRight);
