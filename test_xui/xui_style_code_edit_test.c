...
int main(void)
{
	...
	count=0;
	CHECK(xuiUpdate(ctx,0)==XUI_OK && xuiRender(ctx,target,&damage,1)==XUI_OK,"live assist render");
	...
	count=0;
	CHECK(xuiUpdate(ctx,0)==XUI_OK && xuiRender(ctx,target,&damage,1)==XUI_OK && !has(0x621324ffu) && !has(0x681324ffu),"transparent colors suppress old fills");
	CHECK(xuiStyleClearDefault(ctx)==XUI_OK,"clear");
	count=0;
	CHECK(xuiUpdate(ctx,0)==XUI_OK && xuiRender(ctx,target,&damage,1)==XUI_OK,"restore render");
