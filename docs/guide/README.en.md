# Guides

> English entry generated from the reviewed Chinese documentation. It keeps the same document set and capability boundaries.

[Chinese Source](README.md)

## Documents

- [2.5d Intro](2.5d-intro.en.md)
- [Android Intro](android-intro.en.md)
- [Async Intro](async-intro.en.md)
- [Audio Intro](audio-intro.en.md)
- [Blend Alpha Intro](blend-alpha-intro.en.md)
- [Board Linux Intro](board-linux-intro.en.md)
- [Coordinate Intro](coordinate-intro.en.md)
- [Drawing Intro](drawing-intro.en.md)
- [Error And Log](error-and-log.en.md)
- [First XGE Program](first-xge-program.en.md)
- [Input Intro](input-intro.en.md)
- [iOS macOS Intro](ios-macos-intro.en.md)
- [Miniprogram Intro](miniprogram-intro.en.md)
- [Offscreen Intro](offscreen-intro.en.md)
- [Platform Intro](platform-intro.en.md)
- [Project Structure](project-structure.en.md)
- [Render Target Intro](render-target-intro.en.md)
- [Render Thread Intro](render-thread-intro.en.md)
- [Resource Intro](resource-intro.en.md)
- [Run Mode Intro](run-mode-intro.en.md)
- [Scene Intro](scene-intro.en.md)
- [Shader Material Intro](shader-material-intro.en.md)
- [Text Intro](text-intro.en.md)
- [Texture Intro](texture-intro.en.md)
- [Web Intro](web-intro.en.md)
- [XUI Bridge Intro](xui-bridge-intro.en.md)
- [XUI App UI Intro](xui-app-ui-intro.en.md)
- [XUI Controls Intro](xui-controls-intro.en.md)
- [XUI Input IME Intro](xui-input-ime-intro.en.md)
- [XUI Layout Intro](xui-layout-intro.en.md)
- [XUI Render Intro](xui-render-intro.en.md)
- [XUI Style Inheritance Intro](xui-style-inheritance-intro.en.md)
- [XSON UI Intro](xui-xson-intro.en.md)

## Reading Strategy

Start with the guides to understand the intended workflow, use the API reference for exact C prototypes and lifecycle rules, and use the case documents to connect scripts, source files, and observable results.

## Capability Boundaries

English documents must not claim support that the Chinese documentation does not claim. Platforms that have not been manually validated remain documented as scaffolded, planned, or pending validation.
