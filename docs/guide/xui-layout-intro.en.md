...
> This guide describes the layout API. Widget foundation is organized around widget roles, box model, overflow, clip, Z order, event routing, focus, tab order, IME, and the ScrollModel / ScrollFrame / ScrollView / VirtualView viewport design.

## Layout Model

XUI uses a widget tree. Each widget has a four-layer box model: `outerRect`, `borderRect`, `paddingRect`, and `contentRect`, plus a lightweight `xge_xui_style_t`. `xgeXuiWidgetGetRect` returns the current `borderRect`; use the explicit box rect APIs when code needs a specific layer. Layout runs only when dirty. Reuse widgets and update state instead of rebuilding the tree every frame.
...
`minWidth/minHeight/maxWidth/maxHeight` clamp the assigned size. Row and Column grow distribution handles min/max redistribution. If there is not enough space, overflow is allowed; choose `visible`, `clip`, `hidden`, or explicit ScrollView/ListView behavior.
...
`overflow: scroll` on an ordinary widget does not automatically create a scroll container. Use ScrollView, ListView, TreeView, TableView, or another scrolling control explicitly; those controls mark their widget overflow as `scroll` and clip to the content rect.
...
ListView is for large fixed-height lists. It uses `ScrollModel + ScrollFrame` directly and paints visible rows internally instead of creating widgets for all items:
...
ListView and TreeView use ScrollModel + ScrollFrame directly instead of owning separate scrollbar, clipping, and scroll-boundary logic. TableView uses the same foundation and owns its table-specific column width, synchronized header, merged-cell, and hit-test model.
...
- Do not expect Row or Column overflow to create scrollbars automatically. Choose clip, ScrollView, or ListView.
- Do not build very large lists from thousands of normal child widgets. Use ListView.
...
- `examples/xui_listview_xson`: XSON ListView data, selection, and scrolling.
