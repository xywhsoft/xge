...
`examples/xui_visual_proc_lab` 专门补 XUI 里一批公开的 `procMeasure / procPaint / procEvent` 接口。它把 `Panel / Label / Image / Separator / Button` 组合成一个可自动退出的页签面板，同时直接调用这些公开 proc API 做自检。
...
- `icon=1`：`xgeXuiButtonEventProc` 能处理图标模式按钮的 mouse + keyboard 激活，button paint proc 已绑定。
