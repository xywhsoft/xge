...
`examples/xui_scroll_view_proc_lab` 专门覆盖 `ScrollView` 的 setter/getter，以及直接 `Event / EventProc / PaintProc` 路径。它把 wheel、内容拖拽、竖向滚动条 page jump、横向 thumb drag 串成了一个可自动退出的自检页，同时在窗口里画出真正可滚动的内容块。
...
- `paint=1`：本例已直接调用过 `xgeXuiScrollViewPaintProc`。
...
- `init`、`config`、`wheel`、`drag`、`vbar`、`hbar`、`paint` 均为 `1`。
