...
XSON retained pages attach regular nodes to the active XUI root. Overlay declaration nodes use only the current names, such as `popup` and `msgBox`, and are attached to the overlay root through an overlay portal.
