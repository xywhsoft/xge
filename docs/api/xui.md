...
XSON retained pages attach regular nodes to the active XUI root. Structural overlay controls such as `popup` are attached to the overlay root through an overlay portal. Service overlays such as `msgTip`、`msgBox`、`inputBox` and `toast` are opened by C API callbacks and are not recommended XSON page nodes.
