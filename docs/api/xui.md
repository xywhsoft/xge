...
Overlay Policy 统一 Popup、Menu、ComboBox、Dialog 等浮层的 owner、open、auto close、placement、focus restore 和 z-order 行为。第一版公共策略落在 `xge_xui_popup_t`；Menu、ComboBox 通过内部 Popup 复用该策略，Dialog 保留独立 backdrop 绘制，并接入 focus restore、z base 和 outside close 策略。Tooltip 不再是公开控件，改为每个 widget 的 `tTooltip` 属性和 context 内部 tooltip service。
