...
- `auto close`：`xgeXuiPopupSetAutoClose` 是兼容入口，分别控制 outside click 和 Escape 自动关闭；新代码优先使用 `xgeXuiPopupSetClosePolicy`。
...
`xgeXuiPopupSetAnchorRect` 设置 anchor，未设置时使用 owner rect。`xgeXuiPopupSetOffset` 设置偏移。`xgeXuiPopupSetPlacement` 是旧 placement 模型到 `anchorPoint + direction` 的兼容映射。新设计不缩放内容尺寸；窗口放不下时，Popup 缩小 viewport 并启用内部滚动。`MANUAL` 不自动改动 rect，适合调用方已经完成定位的特殊浮层。
...
| Text | `xgeXuiText*` |
| Controls | `xgeXuiButton*`、`xgeXuiLabel*`、`xgeXuiImage*`、`xgeXuiInput*`、`xgeXuiSlider*`、`xgeXuiProgress*`、`xgeXuiPanel*`、`xgeXuiScrollView*`、`xgeXuiListView*`、`xgeXuiVirtualList*`、`xgeXuiDialog*`、`xgeXuiNumericInput*`、`xgeXuiDatePicker*`、`xgeXuiMessageBox*`、`xgeXuiToolbar*`、`xgeXuiStatusBar*`、`xgeXuiTreeView*`、`xgeXuiTableView*`、`xgeXuiPropertyGrid*`、`xgeXuiBreadcrumb*`、`xgeXuiAccordion*`、`xgeXuiColorPicker*`、`xgeXuiToast*` |
...
### xgeXuiInput Legacy Decoration Wrappers

旧 clear/icon API 仅作为 decoration API 的过渡薄封装保留。新代码和新范例应直接使用 `xgeXuiInputDecorationAdd` 等 API。
