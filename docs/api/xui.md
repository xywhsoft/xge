...
Button、Input、CheckBox、Radio、Switch、Slider、Tabs 使用同一套状态色和圆角；图标按钮由 Button 的图标模式表达。CheckBox/Radio 使用内置 bitmap mask 或几何 fallback 绘制清晰的勾选和圆点；Switch 使用紧凑轨道和 knob，避免旧版过大比例。ListView、TreeView、TableView、PropertyGrid 走高密度工具界面：行高稳定、网格线弱、选中态明显、hover 轻量。Dialog、Menu、ComboBox、Tooltip、Toast 走同一浮层语言：浅 panel、清晰外边框、弱内部分隔。
...
Overlay Policy 统一 Popup、Menu、ComboBox、Dialog 等浮层的 owner、open、close policy、placement、focus restore 和叠放行为。详细设计以 [Popup 文档](../xui/popup.md) 为准。

当前基础入口由 OverlayManager 提供：overlay widget 进入 `xgeXuiOverlayRoot(context)`，默认排序使用 `layer > zIndex > treeOrder`；同层浮层通过 `xgeXuiOverlayBringToFront` 提升。Menu、ComboBox 通过内部 Popup 复用该策略，Tooltip 是每个 widget 的 `tTooltip` 属性和 context 内部提示服务，Dialog 的 backdrop 通过 widget `paintBefore`/underlay 阶段绘制。XSON loader 会把 `type:"popup"`、`type:"dialog"` 和 `type:"messageBox"` 声明节点通过 overlay portal 挂到 overlay root；Menu 的实际浮层通过 MenuInit 接入 overlay root，widget tooltip 的实际浮层通过 context 内部提示服务接入 overlay root。
...
| Controls | `xgeXuiButton*`、`xgeXuiLabel*`、`xgeXuiImage*`、`xgeXuiInput*`、`xgeXuiSlider*`、`xgeXuiProgress*`、`xgeXuiPanel*`、`xgeXuiScrollView*`、`xgeXuiPopup*`、`xgeXuiListView*`、`xgeXuiTreeView*`、`xgeXuiDialog*`、`xgeXuiNumericInput*`、`xgeXuiDatePicker*`、`xgeXuiMessageBox*`、`xgeXuiToolbar*`、`xgeXuiStatusBar*`、`xgeXuiBreadcrumb*`、`xgeXuiAccordion*`、`xgeXuiToast*` |
| Quarantined Viewport Controls | `xgeXuiVirtualList*`、`xgeXuiTableView*`、`xgeXuiPropertyGrid*`、`xgeXuiMenu*`、`xgeXuiComboBox*`、`xgeXuiColorPicker*`、`xgeXuiTextEdit*` |
...
ColorPicker 旧实现当前隔离。恢复时以 [ColorPicker](../xui/colorpicker.md) 和 [Popup](../xui/popup.md) 为准，接入新 Popup 路径，不沿用旧弹层坐标逻辑。
...
## API 参考：Panel / Dialog / Viewport Rebuild
...
ScrollModel、ScrollFrame、ScrollView、Popup、ListView、TreeView 已按新的 viewport 架构落地。

VirtualView、TableView、PropertyGrid、Menu、ComboBox、ColorPicker、TextEdit 正在继续重构。旧 `ScrollViewBase` / `VirtualScrollViewBase` API 不再作为新实现口径，未恢复控件源码已从编译入口隔离。
