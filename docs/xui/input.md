...
Input 是单行文本输入控件。本轮重构保持原有输入、选择、IME、剪贴板、右键菜单、过滤器、错误提示等行为不变，同时把输入框左右装饰区统一为轻量链表机制：

- 边框、背景、hover、focus、disabled 状态交由 widget 状态样式绘制。
- 文本、占位文字、选择区、光标、装饰区、错误文字仍由 Input 自己绘制。
...
- 内容：`text` / `value`、`placeholder`、`textAlign`、`password`、`readonly`、`disabled`、`selection`、`error`、`errorText`
