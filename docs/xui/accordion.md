...
- Header 使用 Button 基础设施，因此具备 pressed、selected、disabled、缓存和文本裁剪能力；Accordion 默认使用淡蓝色 header 和深蓝文字，并保持 hover、pressed、expanded 同色，避免折叠面板成为页面视觉重心。
...
- `headerColor`: header 默认背景色。
- `hoverColor`: 可选。未设置时与 `headerColor` 相同。
- `expandedColor`: 可选。未设置时与 `headerColor` 相同。
- `contentColor`: client 背景色。
- `borderColor`: accordion、header 和 client 边框色。
- `textColor`: header 文本色；展开状态默认仍使用同一个文本色。
