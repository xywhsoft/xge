...
xgeXuiDatePickerSetMode(&date, XGE_XUI_DATE_PICKER_MODE_DATETIME);
xgeXuiDatePickerSetValue(&date, value);
```

NumericInput 负责常见数值录入，支持范围、step、整数/浮点精度、右侧真实 spinner 按钮、滚轮调整、禁用/只读和错误态。ColorPicker 负责基础颜色选择，支持 swatch、RGBA 字段、hex 和 palette；弹层在 context 内复用唯一共享 panel，只在打开时绑定当前控件并同步状态。DatePicker 负责表单化日期时间选择，底层一个控件通过 mode 支持 Date、Time、DateTime 和三类 Range，值统一使用 `xtime`，范围值支持自动交换，`showSecond=false` 时提交秒固定为 0。DatePicker 的弹层在 context 内复用 6 个共享 panel，内部使用标准 Button、Input、ComboBox、NumericInput，日历网格保留为控件自绘区域。

XSON 中分别使用 `type: "numericInput"`、`type: "colorPicker"`、`type: "datePicker"`。DatePicker 的 `value`、`startValue`、`endValue`、`min`、`max` 支持 `xtime` 数值；搜索输入使用 `type: "input"` 搭配 decoration API 或后续 ext 组合控件。ColorPicker 的 `color` 字段表示当前颜色值，文字颜色使用 `textColor`。
...
ScrollView、ListView、TreeView 和 TableView 已复用 ScrollModel / ScrollFrame 新分层。内容拖拽滚动默认关闭，由具体控件显式开启，避免干扰地图、画布、编辑器类控件的鼠标事件。
