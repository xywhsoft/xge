...
本轮重构不保留旧滚动分层。旧 API 和旧实现已删除，后续实现只按这里的边界推进。
...
旧口径中的滚动泛化基类已删除，不再作为所有滚动控件的入口。后续如果需要新的虚拟数据控件抽象，必须基于 `ScrollModel + ScrollFrame` 重新设计，不复用旧 slot 实现。
...
`listView` 已恢复 XSON 支持。它内部直接持有 `ScrollModel + ScrollFrame`，滚动条、滚轮和 viewport 裁剪由 ScrollFrame 处理，行选择和绘制由 ListView 处理。

`treeView` 已恢复 XSON 支持。它内部直接持有 `ScrollModel + ScrollFrame`，滚动条、滚轮和 viewport 裁剪由 ScrollFrame 处理，节点展开、选择、勾选和绘制由 TreeView 处理。

`tableView`、`tableGrid`、`timelineView`、`propertyGrid`、`menu`、`comboBox`、`colorPicker`、`datePicker`、`textEdit` 已按当前控件口径恢复 XSON 支持。字段以新分层为准：
