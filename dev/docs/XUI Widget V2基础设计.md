...
当前 `xgeXuiWidgetSetPaint` 只能替换 widget 的主要 `procPaint`，属于低层回调能力，不等同于完整 OwnerDraw 机制。正式 OwnerDraw 必须显式表达“替换哪一层绘制”，并向回调提供足够上下文，避免控件为了自绘而绕开 Widget 的 clip、focus、disabled、tooltip、debug 和 Z 序基础设施。
...
```c
XGE_XUI_OWNER_DRAW_CONTENT
XGE_XUI_OWNER_DRAW_CONTENT_AND_CHILDREN
XGE_XUI_OWNER_DRAW_FULL
```

- `CONTENT`：只屏蔽控件自己的内容 paint，保留 Widget 基础绘制和 children 绘制。普通控件默认使用此模式。
- `CONTENT_AND_CHILDREN`：屏蔽控件内容和子树绘制，但仍保留 Widget 基础背景、边框、focus ring、disabled overlay、debug outline。用于 Canvas、地图编辑器、复杂自绘面板等。
- `FULL`：屏蔽 Widget 本体全部绘制链，由回调完全接管。该模式只给特殊控件和调试工具使用，默认不建议业务控件使用。

`CONTENT_AND_CHILDREN` 与 `FULL` 跳过子树绘制时，基础层必须清理被跳过子树的 paint dirty 状态。否则子节点会长期保持 dirty，下一次子节点标脏可能无法继续向上冒泡，导致刷新丢失。

OwnerDraw 回调必须携带稳定绘制信息，而不是只传 `widget + user`：
...
typedef struct xge_xui_paint_info_t {
    ...
    int iRow;
    int iColumn;
} xge_xui_paint_info_t;
```

`iState` 至少包含 normal、hover、active、focus、disabled，并允许控件叠加 checked、selected、open、readonly、error 等业务状态。`iPart` 用于区分 content、item、header、cell、thumb、track、icon、text、separator 等绘制区域。基础层只定义通用 part 值；控件可以扩展私有 part，但必须保证文档化。

复合控件不能只暴露 Widget 级 OwnerDraw。它们必须提供控件级接口，例如：
...
这些接口只接管 item/cell 的视觉表现，不改变控件的数据、选择、滚动、虚拟化和事件语义。item/cell OwnerDraw 的回调必须包含逻辑 index，而不是复用 slot index；VirtualScrollViewBase 场景下还必须明确 slot widget 与逻辑 item 的区别。

OwnerDraw 与 XSON 的关系：

- C API 是权威能力面。
- XSON 第一阶段只允许引用已注册的绘制名称，不允许脚本或内嵌绘制代码。
- XSON loader 必须在加载期解析并固化 owner draw 引用，paint 热路径不得查找字符串。
