...
> 2026-05-09 口径更新：MouseEnter、MouseLeave、MouseMove、MouseDown、MouseUp、Click、DoubleClick、ContextMenu、Key、HotKey、Command 等低层语义事件已在 Widget 基础设施实现。本文的控件事件模型只描述 Button/Input/List/Window 等控件业务事件；具体控件后续逐个重构接入新的 Widget 语义事件。
...
- 先完成 Widget V2 基础层，再精修控件。
- 优先提升现有控件质量，再增量加入高价值控件。
- 新增控件优先服务工具型 UI：资源树、属性面板、表格、工具栏、状态栏、消息框。
- 复杂控件必须复用 Widget V2 基础设施，避免每个控件重复实现滚动、弹出层、选择、文本输入和主题逻辑。
- 所有控件必须使用 Widget V2 的 clip、Z、event route、focus、IME、box model、ScrollViewBase / VirtualScrollViewBase。
...
- `open`：ComboBox、Popup、Menu、Tooltip、Window。
...
- Escape 是否取消状态。
- Enter 是否触发默认动作。
- 关闭 Popup/Window 后焦点是否恢复到 owner。
...
- Panel / Window / Window 的外边框必须比内部标题栏分隔线更突出。
...
- `close_8`：Input clear、Tab close、Window close。

## 5. 分层设计

### 5.0 Widget 基础层

Widget 基础层是所有控件成熟度验收的前置条件。旧基础层上的控件实现只算第一版功能，不代表可用于任意场景。

- `Widget`：统一 tree、role、layout、box model、clip、event route、focus、tab、pointer capture、IME、dirty。
- `ScrollViewBase`：统一 viewport、scroll offset、scrollbar、wheel policy、nested scroll 和 ensure visible。
- `VirtualScrollViewBase`：统一可见范围、slot 创建/绑定/回收、selected/hover/focus index、index ensure visible。
- `OverlayManager`：统一 layer、zIndex、modal、popup、tooltip、drag adorner。
- `FocusManager`：统一焦点、focus scope、tab 顺序、默认按钮、取消按钮和焦点恢复。
- `PaintContext`：统一 clip stack、背景、边框、focus ring、debug overlay。
- `OwnerDraw`：统一 Widget 内容自绘和复合控件 item/cell 自绘，避免控件直接覆盖基础绘制链。

控件只有通过 Widget V2 基础层验收后，才进入“成熟可用”状态。
...
Label 作为静态文本原子控件，采用单渲染缓存策略：控件只维护一个缓存纹理，不为 disabled/hover/active/focus 预建多份状态图；文本、字体、颜色、禁用色、对齐、下划线、content 尺寸、DIP scale 或影响视觉的 enabled 状态变化时标记缓存 dirty，下次 paint 惰性重建。无 render target 环境必须回退直接文本绘制。Label 的背景、边框、圆角、padding、clip、tooltip、disabled overlay 和 Widget `CONTENT` OwnerDraw 全部复用 Widget 基础设施。
...
- 动态提示使用 `xgeXuiWidgetSetTooltipResolver(widget, resolver, user)`，由 widget 在命中时按当前状态返回文本或自绘描述；适用于 ListView item、Toolbar item、TableView cell 这类同一控件内部不同区域显示不同提示的场景。
- `xgeXuiWidgetClearTooltip(widget)` 清除静态提示和动态 resolver；`xgeXuiWidgetSetTooltipText(widget, NULL)` 等同于不显示提示。
...
OwnerDraw 是控件成熟度的一部分，但不能替代控件状态机。控件提供自定义绘制时，必须保留自身的数据、选择、滚动、焦点、命中测试、键盘导航、tooltip 和 dirty 规则。

普通控件优先使用 Widget 级 OwnerDraw：

- 默认替换控件内容绘制。
- 保留 Widget 基础背景、content clip、children、disabled overlay、border、focus ring 和 debug outline。
- 回调获得 `xge_xui_paint_info_t`，而不是只有 `widget + user`。
- 只有 Canvas、地图编辑器、特殊 HUD 等场景才使用完整替换模式。

复合控件必须提供 item/cell 级 OwnerDraw，而不是要求用户接管整个控件：

- ListView / VirtualList：item owner draw。
- TreeView：node owner draw。
- TableView：header/cell owner draw。
- PropertyGrid：category/property/value cell owner draw。
- Toolbar / StatusBar / Accordion：item、segment 或 section owner draw。

复合控件 owner draw 回调使用逻辑 index，不使用可复用 slot index。控件仍负责虚拟化、滚动条、选择态、hover/focus、tooltip resolver、键盘导航和事件触发。

### 5.4 数据控件层

数据控件复用 VirtualScrollViewBase、ScrollViewBase、Input、ComboBox 等基础能力：
...
- `ColorPicker`
- `DatePicker`
- 搜索框：不作为 core 控件提供；基础搜索输入由 `Input` decoration 组合表达，带 suggestion popup 的搜索框归 ext/组合控件。
...
- 支持 max length、filter、error text。
- 装饰区从旧的 clear button、prefix icon、suffix icon 特例升级为轻量链表机制：Input 主结构只保存 leading/trailing 两条装饰链表指针、两侧装饰总宽度和 dirty 标记；普通 Input 不为装饰项承担固定节点内存。
- 装饰项由 Input 管理生命周期，调用方通过 desc 添加、更新、删除，不直接分配节点。装饰项支持 icon、text、texture、clear、custom paint 和 click action；clear button 是 trailing decoration 的一种，不再作为独立机制。
- leading 链表按添加顺序从左到右排列；trailing 链表按添加顺序从左到右排列，整个 trailing 区域贴右侧。只有可见装饰项参与宽度计算、绘制和命中。
- 旧 `xgeXuiInputSetClearButton`、`xgeXuiInputSetClearColors`、`xgeXuiInputSetIcons`、`xgeXuiInputSetIconColor` 不再作为新范例和新控件使用口径；如短期保留，只能作为装饰区 API 的薄封装，内部不得维护第二套 clear/icon 状态。
- 密码框的眼睛按钮使用 trailing decoration 表达，默认通过 click action 切换 password 模式；搜索框样式使用 leading search icon + trailing clear decoration 表达。
...
- ScrollViewBase 必须提供 wheelAxis：vertical/y、horizontal/x、both。
- ScrollViewBase 的内容拖拽默认关闭，通过 contentDrag 或 dragMode 显式启用；滚动条 thumb 拖拽默认可用。
- VirtualList、TreeView、TableView、PropertyGrid 必须基于 VirtualScrollViewBase，而不是各自维护一套可见范围、selected/hover/focus index 和 slot 复用逻辑。

### 7.6 Popup / Menu / ComboBox / Window
...
1. 完成 Widget V2 基础层：box model、clip、Z、event route、focus、tab、pointer capture、IME、ScrollViewBase、VirtualScrollViewBase。
2. 把现有控件迁移到 Widget V2，并重新验收渲染裁剪、Z 序、事件、焦点和 IME。
3. 基础应用控件：DatePicker 与 ColorPicker 已补齐完整基础控件能力并重验。
4. 基于 VirtualScrollViewBase 重新验收 TreeView。
5. 基于 VirtualScrollViewBase / ScrollViewBase 重新验收 TableView。
6. 基于已有编辑器控件实现或重验 PropertyGrid。
7. RichTextView、CodeEditor、NodeGraph、Timeline 作为可选高级组件，核心基础层稳定后再推进。
8. 分批把成熟控件接入 XSON。

## 11. 风险

- 过早引入 TableView、PropertyGrid 可能导致 API 固化过快。
- 复杂控件如果不复用 VirtualScrollViewBase / ScrollViewBase，会造成性能和维护成本失控。
- XSON 过早支持复杂控件可能让 loader 膨胀。
- Overlay 规则如果继续分散在各控件里，ComboBox/Menu/Window 行为会长期不一致。
- 主题 token 如果没有语义分层，后续皮肤和状态扩展会困难。
- Widget 基础层如果继续以局部补丁修复 clip、Z、事件和文本溢出，会让控件成熟度长期不可验证。

## 12. 决策

- XUI 控件库继续保持小核心、可组合、可测试，但核心控件范围以 Widget V2 后的通用应用能力为准。
- 第一阶段改为先完成 Widget V2 基础层，再标准化现有控件。
- DatePicker 和 ColorPicker 属于基础应用控件，均已补齐完整基础控件能力；RichTextView、CodeEditor 属于可选高级组件。
- TreeView、TableView、PropertyGrid 是下一阶段工具 UI 能力的核心，但必须建立在 VirtualScrollViewBase、ScrollViewBase、Input、ComboBox 稳定之后。
