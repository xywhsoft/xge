...
- [x] Slider、SplitLayout divider、ScrollBar thumb、拖拽控件改用 pointer capture。说明：Button/IconButton、CheckBox、Radio、Switch、Toggle、StatusBar、Tabs、Toolbar、Input/TextEdit selection、Slider、SplitLayout divider、SplitLayout、Window、ScrollBar、ScrollView、ListView、VirtualList、TreeView、TableView、PropertyGrid 等内置 capture 使用点已按事件 `iPointerId` 设置、查询和释放 capture；内部 active/selection 状态仍为单 pointer 语义，需要多指手势的控件后续单独扩展。
