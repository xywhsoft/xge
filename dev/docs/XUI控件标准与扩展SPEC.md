...
## 阶段 0.7：Input 装饰区重构决策

- [x] 决策：保留 `xgeXuiInputSetFilter` 当前 hook 机制。说明：它以 `oldText/newText` 回调承担 Changing、Filter、Check 的最小灵活性；本轮不新增 allow/deny 内置过滤器和 ASCII bitset 快路径，避免扩大 Input 复杂度。
- [x] 决策：Input 不新增 `onChanging`、`onCancel`、`onCommit` 独立事件。说明：当前 `Change`、`Submit`、`Filter` 足以覆盖本阶段需求；NumericInput 的提交、取消和格式化仍由 NumericInput 自己定义。
- [x] 决策：搜索框不作为 core 控件重构优先项。说明：简单搜索框应由 Input leading search decoration、trailing clear decoration 和 Submit 表达；带 suggestion popup 的搜索框归 ext/范例/组合控件。
- [x] 决策：NumericInput 保留 core value control 定位，但不把 min/max/step/precision、stepper 和数值格式化塞进 Input。说明：NumericInput 后续复用 Input 文本能力和装饰区/组合能力，数值模型归 NumericInput。
- [x] 设计 Input 装饰区内存模型。说明：Input 主结构只保存 leading/trailing 两条链表指针、两侧装饰总宽度和 dirty 标记；不用装饰区的 Input 不分配 decoration 节点。
- [x] 设计 Input 装饰项生命周期。说明：装饰节点由 Input API 创建、更新、删除和清理，调用方只传 desc，不直接 malloc/free 节点。
- [x] 设计装饰项类型。说明：第一版支持 icon、text、texture、clear、custom paint；clear button 是 trailing decoration 的一种，不再作为独立机制。
- [x] 设计装饰项交互。说明：只有存在装饰链表时才进入 decoration hit test；MouseMove 更新 hover，MouseDown 设置 active/capture，MouseUp 命中后触发 click，CaptureLost/Cancel 清理 active。
- [x] 设计装饰区布局。说明：leading 按添加顺序左到右，trailing 按添加顺序左到右并整体贴右；可见装饰项才占宽度，文本区域自动避让两侧总宽。
- [x] 设计装饰区可见性。说明：支持 always、whenNotEmpty、whenFocused、whenFocusedNotEmpty；不可见项不参与布局、绘制和事件。
- [x] 设计旧 API 处理策略。说明：新范例和新控件不再使用 `xgeXuiInputSetClearButton`、`xgeXuiInputSetClearColors`、`xgeXuiInputSetIcons`、`xgeXuiInputSetIconColor`；如短期保留，必须只是新装饰区 API 的薄封装。
- [x] 实现 Input 装饰区 API 和内部布局/事件/绘制。
- [x] 删除或封装旧 clear/icon 特例状态，确保内部只有一套装饰区逻辑。
- [x] 更新 Input 范例：旧图标和 clear button 全部换用新装饰区 API。
- [x] 更新密码框范例：添加 trailing 眼睛按钮 decoration，点击切换 password 显示。
- [x] 增加装饰区事件范例：至少一个 decoration click action 能改变 UI 状态并可自动验证。
- [ ] 增加 Input 回归测试：覆盖无装饰零节点、leading/trailing 多项布局、clear、text/icon/texture/custom paint、hover/active/click、readonly/disabled、visible mode、Unit 清理。
- [x] 更新公开 API 文档和 guide。说明：`docs/api/xui.md` 和 `docs/xui/input.md` 已切到 decoration API；旧 clear/icon API 只作为薄封装口径保留。
...
Overlay Policy 供 Popup、Menu、ComboBox、Window、MsgBox、Toast 共享。Tooltip 从公开控件调整为 widget 内部提示机制，只复用 overlay root、layer/zIndex/treeOrder 和 screen clamp。
...
- [x] 支持非法输入 error 状态。
- [x] 支持 Enter/blur 提交。
- [x] 增加 `xui_numericinput` 和 `xui_numericinput_xson`。
...
- [x] 保证大树只创建可见行。
- [x] 增加 `xui_treeview` / `xui_treeview_xson`。
...
- [x] 搜索框清理为非 core 口径。说明：基础搜索输入由 `Input` decoration 表达；带 suggestion popup 的搜索框归 ext/组合控件。
...
- [x] XSON 支持 `Window`。
- [x] XSON 支持 `MsgBox`。
- [x] XSON `popup`、`Window`、`MsgBox` 声明节点通过 overlay portal 挂到 overlay root；Menu 和 widget tooltip 的实际浮层接入 overlay root。
- [x] XSON 支持 `numericInput`。
- [x] `comboBox` 支持字段：font、items、selected/value、popupHeight、color/background、hoverColor、focusColor、disabledColor、textColor、popupColor、onSelect/onChange 未接入时报错。
...
- [x] XSON 不再支持独立 `type:"tooltip"` 节点；tooltip 仅作为任意 widget 的通用属性加载。
...
- [x] `Window` 支持字段：font、title、open、modal、closeOnEscape、showClose、backdropColor、backgroundColor/background/color、titleColor、closeColor、onClose 未接入时报错。
- [x] `MsgBox` 支持字段：font、title、message/text、kind/messageType/typeName、buttons、open、backdropColor、backgroundColor/background、titleColor、closeColor、messageColor/textColor、buttonColor、buttonHoverColor、buttonTextColor、onResult/onClose 未接入时报错。
