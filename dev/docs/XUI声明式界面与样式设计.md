...
- Control：`label`、`button`、`input`、`searchBox`、`numericInput`、`colorPicker`、`datePicker`、`image`、`separator`、`checkbox`、`radio`、`switch`、`slider`、`progress`、`comboBox`、`tabs`、`toolbar`、`statusBar`、`breadcrumb`、`accordion`。
...
- `searchBox`：绑定 `xge_xui_search_box_t`，支持 `text`/`value`、`placeholder`、`font`、`textColor`、`background`/`backgroundColor`、`focusColor`、`cursorColor`、`placeholderColor`、`selectionColor`、`disabledTextColor`、`disabledBackgroundColor`、`iconColor`、`clearColor`、`readonly`、`disabled`、`selection`、`suggestionsReserved`/`suggestions`。`onSubmit`、`onClear` 和 `onChange` 当前严格拒绝，后续随统一 value/model 绑定落地。
