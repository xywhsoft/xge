...
- 容器：`panel`、`row`、`column`、`stack`、`grid`、`dock`、`scroll`。
- 控件：`label`、`button`、`input`、`image`、`separator`。
- 已有 C 控件扩展：`virtualList` 第一阶段提供固定 item height、可见 slot 复用和 C adapter 绑定；XSON `itemTemplate` 后续接入同一 slot 机制。
- 后续扩展：`window`、`dialog`、`menu`、`popup`、`tooltip`、`tabs`、`split`。
...
第一版 ScrollView 复用 `xge_xui_scroll_view_t`，不引入 CSS overflow 模型。容器默认开启 clip 和 focusable；`contentSize`/`contentWidth`/`contentHeight` 描述虚拟内容尺寸，`offset`/`scrollOffset`/`contentOffset` 或 `scrollX`/`scrollY` 描述初始滚动位置，颜色字段支持 `backgroundColor`、`barColor`、`thumbColor`。布局完成后，ScrollView 只对子 widget 的最终 rect 做滚动偏移，保持 layout 本身单次遍历；hit test 和滚轮只在 content rect 内生效，padding 区域不会触发滚动。
...
- `tokens.colors`：颜色 token，第一版用于 `background` 等颜色字段。
- `tokens.spacing`：间距 token，第一版用于 `width/height/min/max/gap/margin/padding/radius/anchor/grid gap` 等数值或尺寸字段。
