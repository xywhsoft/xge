...
- 2026-05-08：新增 `DatePicker` 基础控件、`datePicker` XSON 接入和 `examples/xui_date_picker_lab`；补齐 `colorPicker` 与 `searchBox` XSON 接入。已通过 `build_dll.bat`、`build_test.bat`、`build_dbg_dll.bat`、`build_dbg_test.bat`、`build_debug_api_compile_test.bat` 和 `build_examples_all.bat --xui`。
