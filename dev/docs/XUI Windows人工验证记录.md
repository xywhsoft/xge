...
- 2026-05-08：补齐 XSON overlay portal 生命周期。`popup`、`dialog`、`messageBox` 声明节点通过 overlay portal 进入 overlay root；Page Loader 通过 path 注册表支持 portaled overlay 的 `PageSyncStyle` 和 `PageUnload` 释放；Menu/Tooltip 的实际浮层继续通过内部 Popup/context tooltip service 接入 overlay root。新增回归覆盖 XSON portal parent、owner/top、style sync、卸载清理和 tooltip owner 清空；复跑 `build_dll.bat`、`build_test.bat`、`build_dbg_dll.bat`、`build_dbg_test.bat`、`build_debug_api_compile_test.bat`、`build_examples_all.bat --xui` 均通过；`git diff --check` 仅有 CRLF 转换提示，代码区 `Widget V2|V2` grep 无匹配。
