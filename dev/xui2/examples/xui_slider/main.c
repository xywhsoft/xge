...
typedef struct xui_slider_demo_t {
	xui_proxy_t tProxy;
	...
	xui_widget pSlider[SLIDER_COUNT];
	int iFrame;
	int iMaxFrames;
	double fMaxSeconds;
	int bHasMouse;
...
static void __xuiSliderUsage(void)
{
	printf("usage: xui_slider [--frames N] [--seconds N]\n");
	printf("       no duration option means run until the window is closed.\n");
}

static int __xuiSliderParseArgs(xui_slider_demo_t* pDemo, int argc, char** argv)
{
	int i;
...
static const char* __xuiSliderFindTtf(void)
{
...
static int __xuiSliderRootRender(xui_widget pWidget, xui_draw_context pDraw, uint32_t iStateId, void* pUser)
{
	xui_slider_demo_t* pDemo;
	xui_rect_t tRect;
...
static int __xuiSliderAddLabel(xui_slider_demo_t* pDemo, int iIndex, const char* sText)
{
...
static int __xuiSliderAddSlider(xui_slider_demo_t* pDemo, int iIndex, int iOrientation, float fValue)
{
	xui_slider_desc_t tDesc;
	xui_widget pSlider;
	int iRet;

	if ( (iIndex < 0) || (iIndex >= SLIDER_COUNT) ) return XUI_ERROR_INVALID_ARGUMENT;
	memset(&tDesc, 0, sizeof(tDesc));
	tDesc.iSize = sizeof(tDesc);
	tDesc.fMin = 0.0f;
	tDesc.fMax = 100.0f;
	...
	iRet = xuiSliderCreate(pDemo->pContext, &pSlider, &tDesc);
	if ( iRet != XUI_OK ) return iRet;
	(void)xuiSliderSetChange(pSlider, __xuiSliderChanged, pDemo);
...
static int __xuiSliderCreateUi(xui_slider_demo_t* pDemo)
{
	int iRet;
	...
	(void)xuiWidgetSetCacheRenderCallback(pDemo->pRoot, __xuiSliderRootRender, pDemo);
	iRet = xuiSetRootWidget(pDemo->pContext, pDemo->pRoot);
	if ( iRet != XUI_OK ) return iRet;
	if ( __xuiSliderAddLabel(pDemo, 0, "Default") != XUI_OK ||
	     __xuiSliderAddSlider(pDemo, 0, XUI_ORIENTATION_HORIZONTAL, 32.0f) != XUI_OK ||
	     __xuiSliderAddLabel(pDemo, 1, "Slim control") != XUI_OK ||
	     __xuiSliderAddSlider(pDemo, 1, XUI_ORIENTATION_HORIZONTAL, 56.0f) != XUI_OK ||
	     __xuiSliderAddLabel(pDemo, 2, "Green accent") != XUI_OK ||
	     __xuiSliderAddSlider(pDemo, 2, XUI_ORIENTATION_HORIZONTAL, 72.0f) != XUI_OK ||
	     __xuiSliderAddLabel(pDemo, 3, "Disabled") != XUI_OK ||
	     __xuiSliderAddSlider(pDemo, 3, XUI_ORIENTATION_HORIZONTAL, 44.0f) != XUI_OK ||
	     __xuiSliderAddLabel(pDemo, 4, "Vertical") != XUI_OK ||
	     __xuiSliderAddSlider(pDemo, 4, XUI_ORIENTATION_VERTICAL, 24.0f) != XUI_OK ||
	     __xuiSliderAddLabel(pDemo, 5, "Vertical accent") != XUI_OK ||
	...
	(void)xuiSliderSetColors(pDemo->pSlider[5],
		...
		XUI_COLOR_RGBA(255, 255, 255, 255),
		XUI_COLOR_RGBA(202, 72, 110, 150),
		XUI_COLOR_RGBA(180, 158, 166, 150));
	(void)xuiSliderSetKnobBorderColor(pDemo->pSlider[5], XUI_COLOR_RGBA(155, 50, 82, 255));
	return XUI_OK;
}

static void __xuiSliderLayout(xui_slider_demo_t* pDemo)
{
	int i;
	float fY;
	...
	for ( i = 0; i < 4; i++ ) {
		...
		(void)xuiWidgetSetRect(pDemo->pLabel[i], (xui_rect_t){28.0f, fY, 140.0f, 24.0f});
		(void)xuiWidgetSetRect(pDemo->pSlider[i], (xui_rect_t){190.0f, fY + 1.0f, 410.0f, (i == 1) ? 20.0f : 26.0f});
	}
	(void)xuiWidgetSetRect(pDemo->pLabel[4], (xui_rect_t){28.0f, 252.0f, 140.0f, 24.0f});
	(void)xuiWidgetSetRect(pDemo->pSlider[4], (xui_rect_t){190.0f, 222.0f, 26.0f, 112.0f});
	(void)xuiWidgetSetRect(pDemo->pLabel[5], (xui_rect_t){280.0f, 252.0f, 150.0f, 24.0f});
	(void)xuiWidgetSetRect(pDemo->pSlider[5], (xui_rect_t){452.0f, 222.0f, 28.0f, 112.0f});
	(void)xuiLayout(pDemo->pContext);
}

static uint32_t __xuiSliderReadButtons(void)
{
	uint32_t iButtons;
...
static int __xuiSliderSendButtonTransitions(xui_slider_demo_t* pDemo, float fX, float fY, uint32_t iButtons, uint32_t iPressed, uint32_t iReleased)
{
	int iRet;
...
static int __xuiSliderSendKey(xui_slider_demo_t* pDemo, int iXgeKey, int iXuiKey)
{
...
static int __xuiSliderHandleInput(xui_slider_demo_t* pDemo)
{
	float fX;
	float fY;
	float fWheelX;
	...
	iButtons = __xuiSliderReadButtons();
	...
	iRet = __xuiSliderSendButtonTransitions(pDemo, pDemo->fUiMouseX, pDemo->fUiMouseY, iButtons, iPressed, iReleased);
	if ( iRet != XUI_OK ) return iRet;
	iRet = __xuiSliderSendKey(pDemo, XGE_KEY_LEFT, XUI_KEY_LEFT);
	if ( iRet == XUI_OK ) iRet = __xuiSliderSendKey(pDemo, XGE_KEY_RIGHT, XUI_KEY_RIGHT);
	if ( iRet == XUI_OK ) iRet = __xuiSliderSendKey(pDemo, XGE_KEY_UP, XUI_KEY_UP);
	if ( iRet == XUI_OK ) iRet = __xuiSliderSendKey(pDemo, XGE_KEY_DOWN, XUI_KEY_DOWN);
	if ( iRet == XUI_OK ) iRet = __xuiSliderSendKey(pDemo, XGE_KEY_PAGE_UP, XUI_KEY_PAGE_UP);
	if ( iRet == XUI_OK ) iRet = __xuiSliderSendKey(pDemo, XGE_KEY_PAGE_DOWN, XUI_KEY_PAGE_DOWN);
	if ( iRet == XUI_OK ) iRet = __xuiSliderSendKey(pDemo, XGE_KEY_HOME, XUI_KEY_HOME);
	if ( iRet == XUI_OK ) iRet = __xuiSliderSendKey(pDemo, XGE_KEY_END, XUI_KEY_END);
	if ( iRet != XUI_OK ) return iRet;
	pDemo->bHasMouse = 1;
...
static void __xuiSliderRunChecks(xui_slider_demo_t* pDemo, int bExerciseInput)
{
	xui_rect_t tTrack;
	xui_rect_t tFill;
	xui_rect_t tKnob;
	...
	tTrack = xuiSliderGetTrackRect(pDemo->pSlider[0]);
	tFill = xuiSliderGetFillRect(pDemo->pSlider[0]);
	tKnob = xuiSliderGetKnobRect(pDemo->pSlider[0]);
	pDemo->bLayoutOK = (xuiWidgetGetRect(pDemo->pSlider[0]).fW > 300.0f) && (xuiWidgetGetRect(pDemo->pSlider[4]).fH > 100.0f);
	pDemo->bDefaultOK = (tTrack.fW > 300.0f) && (tFill.fW > 0.0f) && (tKnob.fW > 10.0f);
	pDemo->bVerticalOK = (xuiSliderGetOrientation(pDemo->pSlider[4]) == XUI_ORIENTATION_VERTICAL) && (xuiSliderGetTrackRect(pDemo->pSlider[4]).fH > 80.0f);
	if ( bExerciseInput && !pDemo->bExerciseDone && pDemo->bLayoutOK ) {
		tWorld = xuiWidgetGetWorldRect(pDemo->pSlider[0]);
		tTrack = xuiSliderGetTrackRect(pDemo->pSlider[0]);
		fX = tWorld.fX + tTrack.fX + tTrack.fW * 0.86f;
...
static int __xuiSliderCreateAssets(xui_slider_demo_t* pDemo)
{
	...
	return __xuiSliderCreateUi(pDemo);
}

static void __xuiSliderDestroyAssets(xui_slider_demo_t* pDemo)
{
...
static int __xuiSliderFrame(void* pUser)
{
	xui_slider_demo_t* pDemo;
	...
	iRet = __xuiSliderHandleInput(pDemo);
	if ( iRet != XUI_OK ) return iRet;
	iRet = xuiDispatchPendingEvents(pDemo->pContext);
	if ( iRet != XUI_OK ) return iRet;
	__xuiSliderLayout(pDemo);
	iRet = xuiUpdate(pDemo->pContext, xgeGetDelta());
	if ( iRet != XUI_OK ) return iRet;
	__xuiSliderRunChecks(pDemo, bAutoRun);
	...
	if ( (pDemo->iMaxFrames > 0 && pDemo->iFrame >= pDemo->iMaxFrames) ||
		...
		printf("xui_slider final-summary frames=%d create=%d layout=%d default=%d vertical=%d input=%d changes=%d updatedCaches=%d drawnCaches=%d cacheSurfaces=%d\n",
			pDemo->iFrame, pDemo->bCreateOK, pDemo->bLayoutOK, pDemo->bDefaultOK, pDemo->bVerticalOK, pDemo->bInputOK,
...
{
	xui_slider_demo_t tDemo;
	xge_desc_t tDesc;
	int iRet;

	memset(&tDemo, 0, sizeof(tDemo));
	iRet = __xuiSliderParseArgs(&tDemo, argc, argv);
	if ( iRet == 1 ) return 0;
	if ( iRet != XGE_OK ) {
		__xuiSliderUsage();
		return 1;
	...
	iRet = xgeInit(&tDesc);
	if ( iRet != XGE_OK ) {
		printf("xui_slider: xgeInit failed: %d\n", iRet);
		return 1;
	}
	iRet = __xuiSliderCreateAssets(&tDemo);
	if ( iRet != XUI_OK ) {
		printf("xui_slider: create assets failed: %d\n", iRet);
		__xuiSliderDestroyAssets(&tDemo);
		xgeUnit();
		return 1;
	}
	iRet = xgeRun(__xuiSliderFrame, &tDemo);
	__xuiSliderDestroyAssets(&tDemo);
	xgeUnit();
	return (iRet == XGE_OK && tDemo.bCreateOK && tDemo.bLayoutOK && tDemo.bDefaultOK && tDemo.bVerticalOK && tDemo.bInputOK) ? 0 : 1;
}
