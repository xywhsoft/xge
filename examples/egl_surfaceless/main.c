...
int main(void)
{
	...
	if ( (tCaps.bCompiled == 0) || (tCaps.bSurfaceless == 0) ) {
		printf("EGL surfaceless is not enabled in this build.\n");
		return 2;
	}
	...
	tDesc.iWidth = 64;
	tDesc.iHeight = 64;
	tDesc.bSurfaceless = 1;
	tDesc.bBoardLinux = 1;
	memset(&tEGL, 0, sizeof(tEGL));
	iRet = xgeEGLInit(&tEGL, &tDesc);
	if ( iRet != XGE_OK ) {
		printf("xgeEGLInit surfaceless failed: %d stage=%s error=0x%04X\n", iRet, tEGL.sLastStage, (unsigned int)tEGL.iLastError);
		return 3;
	}
	iRet = xgeEGLMakeCurrent(&tEGL);
	xgeEGLUnit(&tEGL);
	if ( iRet != XGE_OK ) {
		printf("xgeEGLMakeCurrent surfaceless failed: %d stage=%s error=0x%04X\n", iRet, tEGL.sLastStage, (unsigned int)tEGL.iLastError);
		return 4;
	}
	printf("EGL surfaceless smoke passed.\n");
	return 0;
}
