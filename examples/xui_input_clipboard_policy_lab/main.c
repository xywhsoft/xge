...
static int LoadFont(app_state_t* pApp)
{
	...
	for ( i = 0; i < (int)(sizeof(arrFonts) / sizeof(arrFonts[0])); i++ ) {
		...
		if ( xgeFontLoad(&pApp->tFont, arrFonts[i], 18.0f) == XGE_OK ) {
			pApp->bFontReady = 1;
			printf("xui-input-clipboard-policy-lab font loaded: %s\n", arrFonts[i]);
			return XGE_OK;
		}
	}
	printf("xui-input-clipboard-policy-lab font load failed\n");
...
static void UpdateStatus(app_state_t* pApp)
{
	...
	snprintf(
		sText,
		sizeof(sText),
		"init=%d all=%d copy=%d cut=%d paste=%d readonly=%d clip=%d select=%d..%d text=%s",
		pApp->bInitOK,
		pApp->bSelectAllOK,
		pApp->bCopyOK,
		pApp->bCutOK,
...
static int CreateUI(app_state_t* pApp)
{
	...
	if ( xgeXuiLabelInit(&pApp->tStatusLabel, pApp->pStatusWidget, pFont, "xui input clipboard policy lab") != XGE_OK ) {
		return XGE_ERROR;
	}
	if ( xgeXuiLabelInit(&pApp->tHintLabel, pApp->pHintWidget, pFont, "Auto-check covers Ctrl+A/C/X/V and readonly guard. clip=0 means clipboard fallback path ran.") != XGE_OK ) {
		return XGE_ERROR;
	}
...
static int RunChecks(app_state_t* pApp)
{
	...
	pApp->bCopyOK = 1;
	if ( pApp->bClipboardReady ) {
		pApp->bCopyOK = (strcmp(xgeClipboardGetText(), "alpha") == 0);
	}
	...
	pApp->bCutOK =
		...
		(pApp->iSelectStart == 6) &&
		(pApp->iSelectEnd == 6);
	if ( pApp->bCutOK && pApp->bClipboardReady ) {
		pApp->bCutOK = (strcmp(xgeClipboardGetText(), "beta") == 0);
	}

	xgeXuiInputSetText(&pApp->tInput, "alpha ");
	xgeXuiTextSetCursor(&pApp->tInput.tText, 6);
	if ( pApp->bClipboardReady ) {
		xgeClipboardSetText("BETA");
		MakeKeyEvent(&tEvent, 'V', XGE_KEY_MOD_CTRL);
		(void)xgeXuiInputEvent(&pApp->tInput, &tEvent);
		pApp->bPasteOK = (strcmp(xgeXuiInputGetText(&pApp->tInput), "alpha BETA") == 0);
	} else {
		pApp->bPasteOK = 1;
	}
...
static int AppFrame(void* pUser)
{
	...
	if ( pApp->iFrameCount >= pApp->iFrameLimit ) {
		printf(
			"xui-input-clipboard-policy-lab final-summary frames=%d init=%d all=%d copy=%d cut=%d paste=%d readonly=%d clip=%d select=%d..%d text=%s\n",
			pApp->iFrameCount,
			pApp->bInitOK,
			pApp->bSelectAllOK,
			pApp->bCopyOK,
			...
			pApp->iSelectEnd,
			xgeXuiInputGetText(&pApp->tInput));
		printf("xui-input-clipboard-policy-lab summary frames=%d/%d\n", pApp->iFrameCount, pApp->iFrameLimit);
		xgeQuit();
	}
	return 0;
}
...
int main(int argc, char** argv)
{
	...
	tDesc.iWidth = 780;
	tDesc.iHeight = 240;
	tDesc.sTitle = "XGE XUI Input Clipboard Policy Lab";
	...
	iExitCode = (tApp.bInitOK && tApp.bSelectAllOK && tApp.bCopyOK && tApp.bCutOK && tApp.bPasteOK && tApp.bReadonlyGuardOK) ? 0 : 3;
	AppUnit(&tApp);
	xgeUnit();
	return iExitCode;
}
