...
{
	printf("usage: xui_input_layer [--frames N] [--seconds N]\n");
...
static int __xuiInputDrawAll(xui_input_demo_t* pDemo)
{
	...
	iRet = __xuiInputDrawText(pDemo, "XUI Input Layer", tRect, XUI_COLOR_RGBA(245, 248, 250, 255), XUI_TEXT_ALIGN_LEFT | XUI_TEXT_ALIGN_MIDDLE);
	if ( iRet != XGE_OK ) {
		return iRet;
	}
...
static int __xuiInputCreateAssets(xui_input_demo_t* pDemo)
{
	...
	if ( iRet != XUI_OK ) {
		printf("xui_input_layer: xuiCreate failed: %d\n", iRet);
		return iRet;
	}
	iRet = xuiSetProxy(pDemo->pContext, &pDemo->tProxy);
	if ( iRet != XUI_OK ) {
		printf("xui_input_layer: xuiSetProxy failed: %d\n", iRet);
		return iRet;
	}
	iRet = xuiInputViewport(pDemo->pContext, (float)DEMO_TARGET_W, (float)DEMO_TARGET_H);
	if ( iRet != XUI_OK ) {
		printf("xui_input_layer: xuiInputViewport failed: %d\n", iRet);
		return iRet;
	}
	...
	if ( iRet != XUI_OK ) {
		printf("xui_input_layer: surfaceCreate failed: %d\n", iRet);
		return iRet;
	}
	sFontPath = __xuiInputFindTtf();
	if ( sFontPath == NULL ) {
		printf("xui_input_layer: no usable system TTF font found\n");
	...
	if ( iRet != XUI_OK ) {
		printf("xui_input_layer: fontLoadFile failed: %d\n", iRet);
		return iRet;
	}
...
static int __xuiInputFrame(void* pUser)
{
	...
	iRet = xuiProxyXgePumpInputRect(pDemo->pContext,
		(xui_rect_t){20.0f, 20.0f, (float)DEMO_TARGET_W, (float)DEMO_TARGET_H});
...
int main(int argc, char** argv)
{
	...
	tDesc.iWidth = 800;
	tDesc.iHeight = 460;
	tDesc.sTitle = "XUI Input Layer";
	tDesc.iFlags = XGE_INIT_VSYNC;
	tDesc.iRunMode = XGE_RUN_GAME_LOOP;
	iRet = xgeInit(&tDesc);
	if ( iRet != XGE_OK ) {
		printf("xui_input_layer: xgeInit failed: %d\n", iRet);
		return 1;
	}
