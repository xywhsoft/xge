...
static int LoadFont(app_state_t* pApp)
{
	...
	for ( i = 0; i < (int)(sizeof(arrFonts) / sizeof(arrFonts[0])); i++ ) {
		...
		if ( xgeFontLoad(&pApp->tFont, arrFonts[i], XGE_XUI_DEMO_FONT_SIZE) == XGE_OK ) {
			pApp->bFontReady = 1;
			printf("xui-combo-detail-lab font loaded: %s\n", arrFonts[i]);
			return XGE_OK;
		}
	}
	printf("xui-combo-detail-lab font load failed\n");
...
static void LayoutRoot(app_state_t* pApp)
{
	...
	xgeXuiWidgetSetRect(pApp->pComboWidget, (xge_rect_t){ 28.0f, 82.0f, 240.0f, 34.0f });
	xgeXuiWidgetSetRect(pApp->pHintWidget, (xge_rect_t){ 28.0f, 148.0f, tRoot.fW - 56.0f, 24.0f });
...
static void UpdateStatus(app_state_t* pApp)
{
	...
	snprintf(
		sText,
		sizeof(sText),
		"init=%d config=%d open=%d disabled=%d select=%d key=%d state=%d selected=%d open=%d state=%d cb=%d last=%d",
		pApp->bInitOK,
		pApp->bConfigOK,
		pApp->bOpenOK,
		pApp->bDisabledItemOK,
		pApp->bSelectOK,
		pApp->bKeyboardOK,
		pApp->bStateOK,
...
static int CreateUI(app_state_t* pApp)
{
	...
	if ( xgeXuiLabelInit(&pApp->tStatusLabel, pApp->pStatusWidget, pFont, "xui combo detail lab") != XGE_OK ) {
		return XGE_ERROR;
	}
	...
	if ( xgeXuiLabelInit(&pApp->tHint, pApp->pHintWidget, pFont, "Auto-check covers open/close, disabled row, callback, keyboard open and disabled-state rendering.") != XGE_OK ) {
		return XGE_ERROR;
	}
...
static int RunChecks(app_state_t* pApp)
{
	...
	MakeMouseEvent(
		...
		pApp->tCombo.pListWidget->tContentRect.fY + pApp->tCombo.tList.tBase.fItemHeight * 1.0f + 6.0f);
	iRet = xgeXuiDispatchEvent(&pApp->tXui, &tEvent);
	pApp->bDisabledItemOK =
		(iRet == XGE_XUI_EVENT_CONSUMED) &&
		(xgeXuiComboBoxIsOpen(&pApp->tCombo) != 0) &&
	...
	pApp->bKeyboardOK =
		(iRet == XGE_XUI_EVENT_CONSUMED) &&
		(xgeXuiComboBoxIsOpen(&pApp->tCombo) != 0);
	MakeMouseEvent(&tEvent, XGE_EVENT_MOUSE_DOWN, XGE_MOUSE_LEFT, 560.0f, 220.0f);
...
static int AppFrame(void* pUser)
{
	...
	if ( (pApp->iFrameLimit > 0) && (pApp->iFrameCount >= pApp->iFrameLimit) ) {
		printf(
			"xui-combo-detail-lab final-summary frames=%d init=%d config=%d open=%d disabled=%d select=%d key=%d state=%d selected=%d open=%d state=%d cb=%d last=%d popup_h=%.2f\n",
			pApp->iFrameCount,
			pApp->bInitOK,
			pApp->bConfigOK,
			pApp->bOpenOK,
		...
		printf("xui-combo-detail-lab summary frames=%d/%d\n", pApp->iFrameCount, pApp->iFrameLimit);
		xgeQuit();
	}
	return 0;
}
...
int main(int argc, char** argv)
{
	...
	tDesc.iWidth = 700;
	tDesc.iHeight = 280;
	tDesc.sTitle = "XGE XUI Combo Detail Lab";
	...
	iExitCode =
		(tApp.bInitOK && tApp.bConfigOK && tApp.bOpenOK && tApp.bDisabledItemOK &&
		 tApp.bSelectOK && tApp.bKeyboardOK && tApp.bStateOK) ? 0 : 3;
	AppUnit(&tApp);
	xgeUnit();
	return iExitCode;
}
